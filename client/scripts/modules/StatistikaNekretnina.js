let StatistikaNekretnina = function() {
    ...
    let izvuciGodinu = function(datum) {
        if (!datum) return null;
        if (typeof datum === 'string' && datum.includes('.')) {
            // Format dd.mm.yyyy.
            const dijelovi = datum.split('.');
            const godina = parseInt(dijelovi[2]);
            if (!isNaN(godina)) return godina;
        }
    ...
    let histogramCijena = function(periodi, rasponiCijena) {
        ...
        periodi.forEach((period, indeksPerioda) => {
            let nekretnineUPeriodu = listaNekretnina.filter(nekretnina => {
                let godina = izvuciGodinu(nekretnina.datum_objave);
                return godina !== null && godina >= period.od && godina <= period.do;
            });
