...
const komentari = [
  {
    tekst: 'Odlična lokacija, preporučujem!',
    korisnik_username: 'marko',
    nekretnina_index: 0,
    odgovori: [
      { tekst: 'Hvala na preporuci!', korisnik_username: 'jelena' }
    ]
  },
  {
    tekst: 'Kakvo je stanje fasade na zgradi?',
    korisnik_username: 'petar',
    nekretnina_index: 1,
    odgovori: []
  },
  {
    tekst: 'Da li su dozvoljeni kućni ljubimci?',
    korisnik_username: 'ana',
    nekretnina_index: 2,
    odgovori: [
      { tekst: 'Jesu, naravno.', korisnik_username: 'marko' }
    ]
  }
];
...
async function seedDatabase() {
  try {
    ...
    // Dodaj komentare i njihove odgovore
    for (const kPodatak of komentari) {
      const korisnik = sviKorisnici.find(k => k.username === kPodatak.korisnik_username);
      const nekretnina = sveNekretnine[kPodatak.nekretnina_index];

      if (korisnik && nekretnina) {
        const glavniKomentar = await Komentar.create({
          tekst: kPodatak.tekst,
          KorisnikId: korisnik.id,
          NekretninaId: nekretnina.id,
          KomentarId: null // Glavni komentar nema roditelja
        });

        if (kPodatak.odgovori && kPodatak.odgovori.length > 0) {
          for (const odg of kPodatak.odgovori) {
            const odgKorisnik = sviKorisnici.find(k => k.username === odg.korisnik_username);
            if (odgKorisnik) {
              await Komentar.create({
                tekst: odg.tekst,
                KorisnikId: odgKorisnik.id,
                NekretninaId: nekretnina.id,
                KomentarId: glavniKomentar.id // Veza na roditeljski komentar
              });
            }
          }
        }
