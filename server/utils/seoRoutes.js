...
const { Nekretnina, SlikaNekretnine, Organizacija, sequelize } = require('../models');
...
function installSeoRoutes(app, dist) {
  ...
  app.get('/nekretnine/:slug', async (req, res, next) => { try {
    const property = await Nekretnina.findOne({ where: { slug: req.params.slug, status: { [Op.in]: ['PUBLISHED','SOLD','RENTED','ARCHIVED'] } }, attributes: { exclude: ['punaAdresa','latTacno','lngTacno'] }, include: [{ model: SlikaNekretnine, as: 'Slike', separate: true, limit: 1, order: [['glavna','DESC'],['redoslijed','ASC']] }] });
  ...
  app.get('/sitemap.xml', async (_req,res,next)=>{try{const [properties,agencies]=await Promise.all([Nekretnina.findAll({where:{status:{[Op.in]:['PUBLISHED','SOLD','RENTED','ARCHIVED']},slug:{[Op.ne]:null}},attributes:['slug','updatedAt']}),Organizacija.findAll({where:{status:'VERIFIED'},attributes:['slug','updatedAt']})]);const urls=[{path:'/',updatedAt:null},{path:'/nekretnine',updatedAt:null},...properties.map(x=>({path:`/nekretnine/${encodeURIComponent(x.slug)}`,updatedAt:x.updatedAt})),...agencies.map(x=>({path:`/agencije/${encodeURIComponent(x.slug)}`,updatedAt:x.updatedAt}))];const xml=`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map(x=>`<url><loc>${esc(config.publicBaseUrl+x.path)}</loc>${x.updatedAt?`<lastmod>${new Date(x.updatedAt).toISOString()}</lastmod>`:''}</url>`).join('')}</urlset>`;res.type('application/xml').send(xml)}catch(error){next(error)}});
